...
class HistoryFrame(ctk.CTkFrame):
    ...
    def _addTextBox(cls, parent, row, col):
        ...
        widget.bind('<KeyPress>', lambda event: parent._onKey(widget, event))
        widget.bind('<KeyRelease>', lambda event: parent._onKey(widget, event))
        return widget
    ...
    def _onKey(self, widget, event):
        print(event.type)
        widget.configure(state=ctk.DISABLED if event.type == KEYPRESS_EVENT else ctk.NORMAL)
